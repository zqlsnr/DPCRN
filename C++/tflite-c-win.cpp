...
static void tflite_create(trg_engine* engine)
{

    engine->model_dpcrn = TfLiteModelCreateFromFile("model/dpcrn_quant_new.tflite");
...
void tflite_infer(trg_engine* engine)
{
    ...
    {
        fft_in[i] = engine->in_buffer[i] * win_sin_512[i];
    }
    ...
    memcpy(engine->input_details_1[1]->data.f, engine->states_1, lstm_size * sizeof(float));
    ...
    memcpy(engine->states_1, engine->output_details_1[3]->data.f, lstm_size * sizeof(float));
    ...
    for (int i = 0; i < fft_out_size; i++) {
        ...
        fft_res[i] = complex<double>{ fft_res[i].real() * out_mask[i], fft_res[i].imag() * out_mask[i] };
    }
    

    c2r(shape, strideo, stridel, axes, BACKWARD, fft_res.data(), fft_in, 1.0);
    for (int i = 0; i < block_len; i++)
        estimated_block[i] = (fft_in[i] / block_len) * win_sin_512[i];
...
BYTE* stream_denoise(BYTE* bytes, trg_engine* eng1){
    int bytesLength = BLOCK_SIZE * 2;
    ...
    BYTE* out_bytes = (BYTE*)malloc(BLOCK_SIZE * 2 * sizeof(BYTE));


    for (int i = 0,j = 0; i < bytesLength; i = i + 2)
    {
        inBuffer_s16_16k[j] = bytesToShort(bytes);
        bytes = bytes + 2;
        j++;
    }
...
void audio_denoise_stream(char* in_file, char* out_file, trg_engine* eng1)
{
    ...
    while (!feof(fp))
    {
        ...
        BYTE* out_bytes = stream_denoise(inBuffer_byte_16k, eng1);
        endTime = clock();
        cout << "Running time is: " << static_cast<double>(endTime - startTime) / CLOCKS_PER_SEC * 1000 << "ms" << endl;
        fwrite(out_bytes, BLOCK_SIZE * 2, 1, fp_out);
...
int main(int argc, char* argv[]) {
    printf("audio denoise by DRCRN.\n");
    char* in_file = (char*)"440C020A_mix.wav";
    char* out_file = (char*)"440C020A_mix.pcm";
    
    trg_engine eng1;
    tflite_create(&eng1);
    audio_denoise_stream(in_file, out_file, &eng1);
